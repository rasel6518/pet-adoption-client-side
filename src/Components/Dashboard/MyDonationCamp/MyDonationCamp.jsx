...
const MyDonationCamp = () => {
    ...
    useEffect(() => {
        axiosSecure.get(`/mydonationcamp/${user?.email}`)
            .then(res => {
    ...
    const handleDelete = (id) => {
        ...
        axios.delete(`https://petpals-haven-server.vercel.app/${id}`)
            .then(res => {
    ...
    const columns = [
        ...
        columnHelper.accessor('donationImage', {
            header: 'Pet Image',
            cell: (tableProps) => <img src={tableProps?.row?.original?.donationImage} alt="Pet" className="w-10 h-10 rounded-xl" />,
        }),
        ...
        columnHelper.accessor('actions', {
            ...
            cell: (tableProps) => (
                <div className="flex items-center text-xl gap-2">
                    <Link to={`/dashboard/updatedonationcamp/${tableProps.row.original._id}`}>
